  ...
  return (
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column', overflow: 'hidden', background: BG }}>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: isNarrow ? '1.25rem 1.25rem 3rem' : '1.75rem 2rem 3rem' }}>
        ...
        {/* Categories */}
        <div>
          <p style={{ ...labelSt, marginBottom: '0.9rem', paddingLeft: '0.1rem' }}>Explorer</p>
          <div style={{
            display: 'grid',
...
function CategoryIllustration({ id, color }) {
  const s = { fill: 'none', stroke: color, strokeWidth: 2, strokeLinecap: 'round', strokeLinejoin: 'round' }
  const p = { width: 48, height: 48, viewBox: '0 0 48 48' }
  if (id === 'bases') return (
    <svg {...p}>
      <polygon points="24,4 44,22 24,44 4,22" {...s} />
      <line x1="4" y1="22" x2="44" y2="22" {...s} opacity="0.5" />
      <line x1="24" y1="4" x2="14" y2="22" {...s} opacity="0.5" />
      <line x1="24" y1="4" x2="34" y2="22" {...s} opacity="0.5" />
    </svg>
  )
  if (id === 'ecosysteme') return (
    <svg {...p}>
      <circle cx="24" cy="8" r="4" {...s} />
      <circle cx="8" cy="38" r="4" {...s} />
      <circle cx="40" cy="38" r="4" {...s} />
      <line x1="24" y1="12" x2="10" y2="34" {...s} opacity="0.6" />
      <line x1="24" y1="12" x2="38" y2="34" {...s} opacity="0.6" />
      <line x1="12" y1="38" x2="36" y2="38" {...s} opacity="0.6" />
    </svg>
  )
  if (id === 'transactions') return (
    <svg {...p}>
      <path d="M6,16 L42,16" {...s} />
      <polyline points="34,8 42,16 34,24" {...s} />
      <path d="M42,32 L6,32" {...s} />
      <polyline points="14,24 6,32 14,40" {...s} />
    </svg>
  )
  if (id === 'strategies') return (
    <svg {...p}>
      <rect x="4" y="30" width="8" height="14" fill={color} opacity="0.4" rx="1" />
      <rect x="16" y="20" width="8" height="24" fill={color} opacity="0.6" rx="1" />
      <rect x="28" y="10" width="8" height="34" fill={color} opacity="0.85" rx="1" />
      <rect x="40" y="24" width="8" height="20" fill={color} opacity="0.5" rx="1" />
    </svg>
  )
  if (id === 'securite') return (
    <svg {...p}>
      <path d="M24,4 L40,11 L40,26 C40,35 24,44 24,44 C24,44 8,35 8,26 L8,11 Z" {...s} />
      <polyline points="16,24 21,30 32,18" {...s} />
    </svg>
  )
  if (id === 'analyse') return (
    <svg {...p}>
      <line x1="10" y1="6" x2="10" y2="42" {...s} opacity="0.4" />
      <rect x="6" y="16" width="8" height="16" fill={color} opacity="0.7" rx="1" />
      <line x1="24" y1="10" x2="24" y2="42" {...s} opacity="0.4" />
      <rect x="20" y="22" width="8" height="10" fill={color} opacity="0.45" rx="1" />
      <line x1="38" y1="8" x2="38" y2="42" {...s} opacity="0.4" />
      <rect x="34" y="14" width="8" height="18" fill={color} opacity="0.8" rx="1" />
    </svg>
  )
  if (id === 'psychologie') return (
    <svg {...p}>
      <path d="M4,28 Q10,14 18,28 Q26,42 32,28 Q36,18 44,28" {...s} />
      <path d="M18,28 C18,20 24,14 24,14 C24,14 30,20 30,28" {...s} opacity="0.5" />
    </svg>
  )
  return null
}
...
function CategoryTile({ cat, onClick }) {
  ...
  return (
    ...
    >
      <div style={{ position: 'absolute', top: '0.7rem', right: '0.8rem', opacity: 0.15, pointerEvents: 'none' }}>
        <CategoryIllustration id={cat.id} color={color} />
      </div>
      <div style={{ width: '6px', height: '6px', borderRadius: '50%', background: color, marginBottom: '0.45rem', flexShrink: 0 }} />
      <span style={{ fontFamily: FONT, fontWeight: 700, fontSize: '0.88rem', color: TEXT, lineHeight: 1.2, marginBottom: '0.22rem' }}>
        {cat.label}
      </span>
      <span style={{ fontFamily: MONO, fontSize: '0.55rem', color: hasEntries ? color : TEXT_DIM, opacity: hasEntries ? 0.75 : 0.5 }}>
